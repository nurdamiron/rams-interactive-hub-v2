 ...
 *
 * ВАЖНО для 12V WS2815 (новая лента, dual-signal DI+BI):
 * - 1 адрес = 1 физический LED (индивидуальная адресация, НЕ как WS2811!)
 * - NUM_LEDS = реальное количество диодов на линии
 * - Управляющий сигнал идёт на DI; BI = резервная линия (перемычка на DI в начале)
 */
...
// Для 12V WS2815 (dual-signal DI+BI)
#define LED_PIN           16        // GPIO → DI (data). BI = резерв (перемычка на DI)
#define NUM_LEDS          200       // Реальное число диодов на линии (1 адрес = 1 LED)
#define LED_TYPE          WS2815    // 12V адресная. Нет в FastLED → WS2812B
#define COLOR_ORDER       GRB       // WS2815 обычно GRB

...
void setup() {
  ...
  // Прямое управление пинами Mega (для диагностики и проверки разводки)
  // Пример: /api/pin?mega=2&pin=22&action=ON
  // Пример: /api/pin?mega=2&pin=22&action=OFF
  // Пример: /api/pin?mega=2&pin=22&action=PULSE&dur=1500
  server.on("/api/pin", HTTP_ANY, []() {
    lastRequestTime = millis();
    server.sendHeader("Access-Control-Allow-Origin", "*");
    server.sendHeader("Access-Control-Allow-Methods", "GET, POST, OPTIONS");
    server.sendHeader("Access-Control-Allow-Headers", "Content-Type");
    ...
    int megaNum  = server.arg("mega").toInt();
    int pin      = server.arg("pin").toInt();
    String action = server.arg("action");
    int dur = server.hasArg("dur") ? server.arg("dur").toInt() : 1000;

    if (megaNum != 1 && megaNum != 2) {
      server.send(400, "text/plain", "ERROR:mega must be 1 or 2");
      return;
    }
    if (pin < 22 || pin > 53) {
      server.send(400, "text/plain", "ERROR:pin must be 22-53");
      return;
    }
    ...
    if (action == "ON") {
      ...
      if (dur <= 0) dur = 1000;
      cmd = "PIN:" + String(pin) + ":ON:" + String(dur);
    } else {
      server.send(400, "text/plain", "ERROR:action must be ON/OFF/PULSE");
      return;
    }

    if (megaNum == 1) {
      Mega1Serial.println(cmd);
      Serial.println("[MEGA1 PIN TX] " + cmd);
    } else {
      Mega2Serial.println(cmd);
      Serial.println("[MEGA2 PIN TX] " + cmd);
    }
